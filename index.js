...
function productBtnDefined(data){
    ...
    data.forEach((item) => {
        ...
        `
            <button onclick = "productCategory()" class=" btn hover:border-[#422AD5] hover:bg-[#422AD5] hover:text-white">${item} </button>
        `

        productBtnID.appendChild(createButton);
    } )
...
function productCategory (){
    ...
    fetch(url)
    .then((res) => res.json())
    .then((data) => productCategoryCard(data))
}

function productCategoryCard (categorys){
